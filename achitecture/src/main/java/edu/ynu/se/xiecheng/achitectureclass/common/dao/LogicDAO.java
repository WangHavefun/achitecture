...
public interface LogicDAO<T extends LogicEntity, ID extends Serializable> extends JpaRepository<T, ID> {
    ...
    @Query(value =
            "update #{#entityName} e " +
                    "set e.deletedTime = current_timestamp " +
                    "where e.id = ?1 and e.deleteTime = null")
    @Transactional
    @Modifying
    ...
    @Override
    @Query(value = "update #{#entityName} e set e.deletedTime = current_timestamp where e.deleteTime = null")
    @Transactional
    @Modifying
    void deleteAll();
    ...
    @Query("select count(e) from #{#entityName} e where e.deleteTime = null")
    long count();
